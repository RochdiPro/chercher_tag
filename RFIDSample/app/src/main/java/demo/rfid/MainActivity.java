       ...
       // textViewConnectedDevice = (TextView) findViewById(R.id.textView_connected_device);
        ...
        btnexporterdata.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                data.append( "," );
                data.append(  "Nom" );
                ArrayList article_code = new ArrayList();
                ArrayList article_nom = new ArrayList();
                try {
                    InputStream inputStream = getAssets().open("db.csv");
                    int size = inputStream.available();
                    byte[] buffer = new byte[size];
                    inputStream.read(buffer);
                    String[] t = new String(buffer).split("\n");
                    for( int i= 1 ; i < t.length ; i++)
                    {
                        String[] t2 = t[i].split(";");
                        article_code.add(t2[1]);
                        article_nom.add(t2[0]);
                ...
                for (int i  =0 ; i<baseAdapterInventoryListView.getCount(); i++) {
                    ...
                    String art ="inconnu";
                    for (int cc  =0 ; cc<article_code.size(); cc++) {
                        if(article_code.get(cc).equals(""))
                        {
                            art=article_nom.get(cc)+"";
                        }
                    }
